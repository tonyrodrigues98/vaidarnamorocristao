...
├─ blog/{,$slug}
├─ v2/{,$section}
└─ 29 rotas planas de produto/conteúdo
```
...
| `/membros`                        | `membros.tsx` / CommunityMembersAlias      | Herd                   | nenhum backend                                                      | shell V2; flags decidem o destino             | descoberta comunitária V2 / L2              |
...
| `/v2/$section`                    | `v2.$section.tsx`                          | Auth + flag; `section` | Início/descoberta usam adapters V2; demais páginas são provisórias  | registry V2; not-found localizado             | migração vertical por domínio / L3          |
...
| `/membros`                    | `/v2/explorar-pessoas` ou `/inicio` | flags V2/community | alias comunitário com fallback legado           |
...
O scan encontrou **489 referências tipadas**:

| Dimensão      | Totais                                                                               |
| ------------- | ------------------------------------------------------------------------------------ |
| origem        | `src` 437; testes 17; manifest 16; sitemap 11; public 8; configuração 0; outras 0    |
| classificação | rotas 428; assets 38; endpoints 4; externas 12; deep links 4; dinâmicas 3            |
| estado        | resolvidas 428; não resolvidas 0; exige investigação 3; não aplicável a resolução 58 |
...
No estado atual, nenhuma referência classificada como rota está sem resolução. `/membros` foi
resolvido por um alias controlado, sem remover o fallback legado. Esse é um resultado regenerável,
não um contrato de teste: a suíte admite zero links não resolvidos. Três destinos dinâmicos
