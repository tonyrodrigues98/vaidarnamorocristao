...
export async function getMyCoins(): Promise<CoinsStatus> {
  const { data, error } = await supabase.rpc("get_my_coins" as never);
  if (error) throw error;
  const row = Array.isArray(data) ? (data[0] as CoinsStatus) : (data as CoinsStatus);
  return {
...
export async function claimDailyCoins(): Promise<{ balance: number; awarded: number }> {
  const { data, error } = await supabase.rpc("claim_daily_coins" as never);
  if (error) throw error;
  const row = Array.isArray(data) ? (data[0] as { balance: number; awarded: number }) : (data as { balance: number; awarded: number });
