  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          <ArcadeMetric
            label={condition === "above" ? "Condição" : "Condição"}
            value={condition === "above" ? `Acima de ${target}` : `Abaixo de ${target}`}
            tone="dark"
          />
          <ArcadeMetric label="Estimativa" value={`${estimatedMultiplier.toFixed(2)}x`} />
        </div>
        {!result || rolling ? (
          <>
            <div className="grid grid-cols-2 gap-2">
