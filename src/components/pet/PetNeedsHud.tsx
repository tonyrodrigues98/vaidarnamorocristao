      ...
      {PET_CARE_ORDER.map((k) => {
        ...
        return (
          <button
            ...
            className={cn(
              "group flex min-w-0 flex-col gap-1 rounded-2xl border border-neutral-200 bg-white/90 px-2 py-1.5 text-left backdrop-blur transition",
              onPick && "hover:border-neutral-300",
            )}
            aria-label={`${PET_CARE_LABEL[k]} ${v}%`}
          >
            <div className="flex items-center justify-between gap-1">
              <Icon className={cn("h-3.5 w-3.5 shrink-0", low ? "text-red-500" : "text-neutral-600")} />
              <span className={cn("text-[10px] font-semibold tabular-nums", low ? "text-red-600" : "text-neutral-700")}>
                {v}
              </span>
            </div>
            ...
            <span className="truncate text-[9px] font-medium uppercase tracking-wide text-neutral-400">
              {PET_CARE_LABEL[k]}
            </span>
          </button>
        );
