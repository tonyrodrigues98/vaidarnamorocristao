        ...
        {!cfg ? (
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              ...
              <Input
                value={String(cfg.default_free_daily)}
                inputMode="numeric"
            ...
            <div>
              ...
              <Input
                value={String(cfg.default_paid_cost_coins)}
                inputMode="numeric"
...
function PoolEditorDialog({
  ...
  async function patchPrize(id: string, patch: Partial<GrabPoolPrize>) {
    ...
    try {
      await updateGrabPrize(id, patch);
    } catch (e) {
      toast.error((e as Error).message);
    }
  }
  ...
  async function removePrize(id: string) {
    ...
    try {
      await deleteGrabPrize(id);
      setPrizes((a) => a.filter((p) => p.id !== id));
    } catch (e) {
      toast.error((e as Error).message);
    }
  }
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <Input
              value={draft.cost_coins == null ? "" : String(draft.cost_coins)}
              inputMode="numeric"
              placeholder="padrão da config"
              onChange={(e) =>
                setDraft({
                  ...draft,
                  cost_coins: e.target.value === "" ? null : toInt(e.target.value),
                })
              }
            />
          </div>
          <div>
            ...
            <Input
              value={draft.free_daily_uses == null ? "" : String(draft.free_daily_uses)}
              inputMode="numeric"
              placeholder="padrão da config"
              onChange={(e) =>
                setDraft({
                  ...draft,
                  free_daily_uses: e.target.value === "" ? null : toInt(e.target.value),
                })
              }
            />
          </div>
...
function PrizeRow({
  ...
  const needsRef = ["care_item", "pet_background", "decoration", "name_gradient"].includes(
    prize.prize_kind,
  );
  ...
  useEffect(() => {
    ...
    listPrizeCatalog(prize.prize_kind)
      .then(setCatalog)
      .catch(() => setCatalog([]));
  }, [prize.prize_kind, needsRef]);

  const pct =
    totalWeight > 0 && prize.active ? ((prize.weight / totalWeight) * 100).toFixed(1) : "0";
  ...
  return (
    <li className="grid grid-cols-12 items-end gap-2 rounded-xl border border-neutral-200 p-2">
      <div className="col-span-12 sm:col-span-3">
        ...
        <Select
          value={prize.prize_kind}
          onValueChange={(v) => onChange({ prize_kind: v as GrabPrizeKind, prize_ref_id: null })}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
      ...
      <div className="col-span-12 sm:col-span-4">
        ...
        {needsRef ? (
          <Select
            value={prize.prize_ref_id ?? ""}
            onValueChange={(v) => onChange({ prize_ref_id: v })}
          >
            <SelectTrigger>
      ...
      <div className="col-span-4 sm:col-span-1">
        ...
        <Input
          value={String(prize.prize_amount)}
          inputMode="numeric"
      ...
      <div className="col-span-4 sm:col-span-1 text-center">
        ...
        <div className="flex h-9 items-center justify-center rounded-md bg-neutral-50 text-xs font-semibold">
          <Percent className="size-3 mr-0.5 text-neutral-400" />
          {pct}
        </div>
      </div>
