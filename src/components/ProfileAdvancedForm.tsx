...
import { Save, Sparkles, HeartHandshake, Users, Compass, Target, Smile, Music, Clock, MessageCircleHeart, Search, BookHeart } from "lucide-react";
import {
  EMPTY_ADVANCED, type AdvancedProfile,
...
function ChipSingle({ value, options, onChange }: { value: string | null; options: Opt[]; onChange: (v: string | null) => void }) {
  return (
...
function ChipMulti({ values, options, onChange }: { values: string[]; options: Opt[]; onChange: (next: string[]) => void }) {
    ...
    <form onSubmit={save} className="space-y-5">
      <div className="rounded-2xl border border-[var(--rose)]/20 bg-[var(--rose)]/5 p-4 text-sm">
        ...
        <p className="mt-1 text-muted-foreground">
          Preencha o que fizer sentido. Seções vazias somem do seu perfil público.
          Ao completar 8 das 10 seções você ganha a badge <strong>Perfil Avançado</strong>.
        </p>
      </div>
      ...
      <Section icon={<BookHeart className="h-4 w-4" />} title="Identidade Espiritual" subtitle="Quem você é em Cristo, hoje">
        ...
        <Field label="Momento da fé">
          <ChipSingle value={data.faith_moment} options={FAITH_MOMENT} onChange={(v) => set("faith_moment", v)} />
        </Field>
      ...
      <Section icon={<HeartHandshake className="h-4 w-4" />} title="Estilo de Vida Cristão" subtitle="Como sua fé acontece no dia a dia">
        ...
        <Field label="Sua rotina espiritual">
          <ChipMulti values={data.spiritual_routine ?? []} options={SPIRITUAL_ROUTINE} onChange={(v) => set("spiritual_routine", v)} />
        </Field>
      ...
      <Section icon={<Users className="h-4 w-4" />} title="Ministério e Chamado">
        ...
        {data.ministry === "outro" && (
          <Field label="Qual?">
            <Input value={data.ministry_other ?? ""} onChange={(e) => set("ministry_other", e.target.value || null)} maxLength={80} />
          </Field>
        )}
        <Field label="Sente um chamado?">
          <ChipSingle value={data.has_calling} options={HAS_CALLING} onChange={(v) => set("has_calling", v)} />
        </Field>
        {data.has_calling === "sim" && (
          <Field label="Conte um pouco">
            <Textarea rows={2} maxLength={300} value={data.calling_description ?? ""} onChange={(e) => set("calling_description", e.target.value || null)} />
          </Field>
        )}
      </Section>
      ...
      <Section icon={<Compass className="h-4 w-4" />} title="Visão de Futuro">
        ...
        <Field label="Quer filhos?">
          <ChipSingle value={data.wants_children} options={SIM_NAO_TALVEZ} onChange={(v) => set("wants_children", v)} />
        </Field>
        {data.wants_children === "sim" && (
          <Field label="Quantos (opcional)">
            <Input
              ...
              onChange={(e) => set("children_count", e.target.value ? Number(e.target.value) : null)}
              className="max-w-[120px]"
            />
          </Field>
        )}
        <Field label="Onde deseja viver">
          <ChipSingle value={data.living_place} options={LIVING_PLACE} onChange={(v) => set("living_place", v)} />
        </Field>
      ...
      <Section icon={<Music className="h-4 w-4" />} title="Gostos e Interesses">
        ...
        <Field label="Louvores favoritos">
          <Input value={data.favorite_worships ?? ""} onChange={(e) => set("favorite_worships", e.target.value || null)} maxLength={200} />
        </Field>
        <Field label="Estilo de culto preferido">
          <ChipSingle value={data.worship_style} options={WORSHIP_STYLE} onChange={(v) => set("worship_style", v)} />
        </Field>
      ...
      <Section icon={<Search className="h-4 w-4" />} title="O que busco" subtitle="Seja honesto — isso atrai conexões reais">
        ...
        <Field label="Algo que não abre mão">
          <Input value={data.non_negotiable ?? ""} onChange={(e) => set("non_negotiable", e.target.value || null)} maxLength={120} />
        </Field>
