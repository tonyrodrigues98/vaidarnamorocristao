...
export function useLongPress(onLongPress: () => void, delay = 450) {
  ...
  const endTouch = useCallback((e: React.TouchEvent) => {
    if (triggered.current) {
      // Prevent only the compatibility click generated by this same touchend.
      // A global click suppressor would also eat the user's first real tap on
      // the freshly opened action menu on some mobile browsers.
      e.preventDefault();
      e.stopPropagation();
    }
    clear();
