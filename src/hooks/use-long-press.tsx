...
export function useLongPress(onLongPress: () => void, delay = 450) {
  ...
  const endTouch = useCallback(() => {
    ...
    //
    // After the long press fires, the action menu opens above the bubble.
    // The user typically lifts the finger ON a menu button (or slides to it).
    // The browser dispatches a synthetic click at the touchend coordinates,
    // which lands on whatever element is currently under the finger — i.e.
    // the menu button. Calling preventDefault would suppress that click and
    // make the first tap appear to "do nothing".
    //
    // We rely on `touch-action: none` on the bubble to prevent scrolling
    // during the long press, and on the menu being rendered above the
    // overlay (z-50) so the synthetic click reaches the right target.
    clear();
