...
function List() {
  ...
  if (nativeShellActive) {
    const model: NativeConversationsViewModel = {
      query,
      items,
      filteredItems,
      showCommunity,
  ...
  return (
    <LegacyConversationsView
      ...
      onQueryChange={setQuery}
      onRefresh={refetch}
      refreshDisabled={!user || !isOnline}
    />
  );
}

type LegacyConversationsViewProps = {
  query: string;
...
function LegacyConversationsView({
  query,
  items,
  filteredItems,
  showCommunity,
