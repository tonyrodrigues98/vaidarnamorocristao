...
function TicketPage() {
    ...
    const load = async () => {
      ...
      if (!ignore) {
        ...
        const ids = Array.from(
          new Set([
            ...(t
              ? ([(t as Ticket).user_id, (t as Ticket).assigned_to].filter(Boolean) as string[])
              : []),
        ...
        if (paths.length) {
          ...
          if (urls && !ignore) {
            ...
            urls.forEach((u, i) => {
              if (u.signedUrl) map[paths[i]] = u.signedUrl;
            });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const list: StaffOption[] = (roles ?? [])
        .map((r: any) => {
          const p: any = profMap.get(r.user_id);
          return p
            ? { user_id: r.user_id, full_name: p.full_name, photo_url: p.photo_url, role: r.role }
            : null;
        })
    ...
    return (
      <div className="min-h-screen">
        ...
        <main className="mx-auto max-w-2xl px-4 py-10 text-center">
          ...
          <Link to="/suporte" className="text-[var(--rose)] hover:underline">
            Voltar
          </Link>
        </main>
      </div>
        ...
        <div className="glass mt-3 rounded-2xl p-5 shadow-soft">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 flex items-center gap-2">
                <Avatar className="h-7 w-7">
                  ...
                  <AvatarFallback>
                    {(profiles[ticket.user_id]?.full_name ?? "?").charAt(0)}
                  </AvatarFallback>
                </Avatar>
                <span className="text-sm font-medium">
                  {profiles[ticket.user_id]?.full_name ?? "Usuário"}
                </span>
              ...
              {ticket.assigned_to && (
                <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  {profiles[ticket.assigned_to]?.full_name ?? "—"}
                </p>
              )}
            </div>
          ...
          {isStaff && (
            <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
              <Select
                value={ticket.status}
                onValueChange={(v) => updateField({ status: v as Ticket["status"] })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <Select
                ...
                onValueChange={(v) => updateField({ priority: v as Ticket["priority"] })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <Select
                ...
                onValueChange={(v) => updateField({ category: v as Ticket["category"] })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <Select
                ...
                onValueChange={(v) => assignTo(v === "__none__" ? null : v)}
              >
                <SelectTrigger>
          ...
          {msgs.map((m) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`max-w-[78%] rounded-2xl px-4 py-2 text-sm shadow-soft ${
                    m.is_staff
                ...
                >
                  <p className="mb-0.5 flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide opacity-80">
                    ...
                    <span className={m.is_staff ? "text-[var(--rose)]" : ""}>
                      {sender?.full_name ?? (mine ? "Você" : "Usuário")}
                    </span>
                      ...
                      {m.attachments.map((a) => (
                        ...
                        >
                          {signed[a.path] ? (
                            ...
                            <span className="flex h-24 w-24 items-center justify-center text-xs">
