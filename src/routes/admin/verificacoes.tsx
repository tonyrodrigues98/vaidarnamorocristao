...
function AdminVerifs() {
  ...
  useEffect(() => { if (canAccess) load(); }, [canAccess, tab]);

  async function preview(path: string, title: string) {
    const { data, error } = await supabase.storage.from("verifications").createSignedUrl(path, 60 * 5);
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {req.document_type.toUpperCase()} · enviado em {new Date(req.created_at).toLocaleString("pt-BR")}
                    </p>
                  </div>
                  <Link to="/pretendentes/$id" params={{ id: req.user_id }} className="text-xs text-primary hover:underline">
                    Ver perfil
                  </Link>
                </div>

                <div className="mt-4 grid gap-2 sm:grid-cols-2">
                  <Button variant="outline" size="sm" onClick={() => preview(req.selfie_path, "Selfie")}>
                    <Eye className="mr-2 h-4 w-4" /> Ver selfie
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => preview(req.document_path, "Documento")}>
                ...
                {tab === "pending" || tab === "more_info" ? (
                  <>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Button size="sm" disabled={busy === req.id} onClick={() => decide(req, "approved")}>
                        <Check className="mr-1 h-4 w-4" /> Aprovar
                      </Button>
                      <Button size="sm" variant="outline" disabled={busy === req.id} onClick={() => decide(req, "more_info")}>
                        <RefreshCw className="mr-1 h-4 w-4" /> Pedir mais info
                      </Button>
                      <Button size="sm" variant="destructive" disabled={busy === req.id} onClick={() => decide(req, "rejected")}>
        ...
        <DialogContent className="max-w-3xl">
          ...
          {previewing && (
            previewing.url.toLowerCase().includes(".pdf")
              ...
              : <img src={previewing.url} alt={previewing.title} className="max-h-[70vh] w-full rounded-lg object-contain" />
          )}
        </DialogContent>
      </Dialog>
    </div>
