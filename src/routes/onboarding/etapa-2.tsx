...
}).refine((d) => d.age_max >= d.age_min, { message: "Idade máxima deve ser maior", path: ["age_max"] });
...
function Etapa2() {
  ...
  const [form, setForm] = useState({
    ...
    desired_quality: "", accepts_children: "sim" as "sim" | "nao",
    looking_for_bio: "",
  });

  useEffect(() => {
    if (!user) return;
    supabase.from("profile_preferences").select("*").eq("user_id", user.id).maybeSingle().then(({ data }) => {
      if (data) {
        setForm({
          age_min: String(data.age_min), age_max: String(data.age_max),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("profile_preferences").upsert({
      ...
      custom_states: parsed.data.location_scope === "personalizado" ? parsed.data.custom_states : [],
        ...
        <form onSubmit={handleSubmit} className="glass animate-fade-up space-y-6 rounded-3xl p-8 shadow-elegant">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input type="number" min={18} max={110} value={form.age_min} onChange={(e) => setForm({ ...form, age_min: e.target.value })} required />
            </div>
            <div className="space-y-2">
              <Label>Idade máxima</Label>
              <Input type="number" min={18} max={110} value={form.age_max} onChange={(e) => setForm({ ...form, age_max: e.target.value })} required />
            </div>
          </div>

          <div className="space-y-3">
            <Label>Localização desejada</Label>
            <RadioGroup value={form.location_scope} onValueChange={(v) => setForm({ ...form, location_scope: v as typeof form.location_scope })}>
              {[
          ...
          <div className="space-y-2">
            ...
            <Input value={form.desired_quality} onChange={(e) => setForm({ ...form, desired_quality: e.target.value })} placeholder="Ex: temor a Deus, paciência, integridade..." />
          </div>

          <div className="space-y-3">
            <Label>Aceita pessoa com filhos?</Label>
            <RadioGroup value={form.accepts_children} onValueChange={(v) => setForm({ ...form, accepts_children: v as "sim" | "nao" })} className="flex gap-3">
          ...
          <div className="space-y-2">
            ...
            <Textarea rows={4} maxLength={600} value={form.looking_for_bio} onChange={(e) => setForm({ ...form, looking_for_bio: e.target.value })} />
          </div>
