...
function OnboardingFlow() {
  ...
  const [skipForOldUser, setSkipForOldUser] = useState(false);

  // If the user already has a complete profile, skip the onboarding entirely.
  useEffect(() => {
    if (!user) return;
    supabase
      .from("profiles")
      .select("id, full_name, age, sex, photo_url")
      .eq("id", user.id)
      .maybeSingle()
      .then(({ data }) => {
        if (data && data.full_name && data.age && data.sex && data.photo_url) {
          setSkipForOldUser(true);
          setStep(5);
        }
      });
  }, [user]);
  ...
  return (
    ...
    >
      ...
      <main className="flex flex-1 flex-col px-6">
        {step === 1 && (
          <StepName name={name} onChange={setName} onSubmit={goNext} />
        )}
        {step === 2 && (
        ...
        {step === 5 && (
          <StepWelcome
            name={skipForOldUser ? "" : name.trim()}
            onContinue={() => navigate({ to: "/inicio" })}
            onCompletePerfil={() =>
              navigate({ to: skipForOldUser ? "/perfil" : "/onboarding/etapa-1" })
            }
          />
        )}
      </main>
...
function Wheel({
  ...
  return (
    <div className="relative" style={{ height: ITEM_H * VISIBLE }} aria-label={ariaLabel}>
      <div
        className="pointer-events-none absolute inset-x-1 top-1/2 -translate-y-1/2 rounded-xl border-y border-[var(--rose-soft)]/50 bg-[var(--rose)]/5"
        style={{ height: ITEM_H }}
      />
      <div
        ref={ref}
        ...
        {items.map((it) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex w-full snap-center items-center justify-center text-base transition",
          ...
          return (
            ...
            >
              <Icon
                className={cn(
                  "h-9 w-9",
                  active ? "text-[var(--rose)]" : "text-muted-foreground",
                )}
              />
              <span
                className={cn(
...
function StepPhoto({
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center py-6">
      <h1 className="self-start text-3xl font-semibold leading-tight">
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-8 text-center">
        ...
        {firstName && (
          <h2 className="mt-2 text-4xl font-light text-white/95">{firstName}</h2>
        )}
        <p className="mt-6 max-w-sm text-base text-white/90">
          Seu perfil já começou. Agora você pode entrar no app ou completar mais
