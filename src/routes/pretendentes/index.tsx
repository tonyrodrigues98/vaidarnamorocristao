    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-4 py-10">
        <div className="animate-fade-up">
          <h1 className="font-serif text-4xl font-semibold">Pretendentes</h1>
        ...
        {myStatus !== "approved" ? (
          ...
          <div className="glass mt-8 rounded-2xl p-12 text-center shadow-soft">
            <p className="font-serif text-xl">Ainda não há pretendentes para mostrar.</p>
                ...
                <div className="relative aspect-[4/5] overflow-hidden bg-muted">
                  {p.photo_url ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[var(--gold-soft)] to-[var(--accent)]">
                      <span className="font-serif text-5xl text-white">{p.full_name.charAt(0)}</span>
                    </div>
                  )}
                </div>
                <div className="p-5">
                  <h3 className="font-serif text-xl font-semibold">{p.full_name.split(" ")[0]}, {p.age}</h3>
                  <p className="mt-0.5 text-sm text-muted-foreground">{p.city} · {p.state}</p>
                  <p className="mt-1 text-xs text-[var(--gold)]">{p.church}</p>
