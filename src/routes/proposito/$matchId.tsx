...
function CouplePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const a = profiles?.find((p) => p.id === commitment.user_a) ?? null;

      const b = profiles?.find((p) => p.id === commitment.user_b) ?? null;
      ...
      <main className="mx-auto max-w-4xl px-4 py-10">
            ...
            <div className="text-center">
              <div className="mx-auto mb-2 flex h-24 w-24 items-center justify-center">
                <DecoratedAvatar
                  ...
                  fallback={personA?.full_name?.charAt(0) ?? "?"}
                  size={96}
                  frameId={personA?.equipped_frame_id ?? null}
                  auraId={personA?.equipped_aura_id ?? null}
                />
              </div>
            ...
            <div className="text-center">
              <div className="mx-auto mb-2 flex h-24 w-24 items-center justify-center">
                <DecoratedAvatar
                  ...
                  fallback={personB?.full_name?.charAt(0) ?? "?"}
                  size={96}
                  frameId={personB?.equipped_frame_id ?? null}
                  auraId={personB?.equipped_aura_id ?? null}
                />
              </div>

              <p className="font-semibold">{personB?.full_name ?? "—"}</p>
            </div>
          </div>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          ...
          <div className="glass rounded-3xl p-6 text-center shadow-soft">
            ...
            <div className="text-3xl font-bold">{giftCount}</div>

            <p className="text-sm text-muted-foreground">Presentes trocados</p>
          </div>
        </section>
        ...
        <section className="mt-8">
          <div className="glass rounded-3xl p-6 shadow-soft">
            <div className="mb-6 flex items-center gap-2">
              ...
              <h2 className="font-semibold">Conquistas do Casal</h2>
            </div>
            ...
            <div className="space-y-6">
              {matchCreatedAt && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-medium">Match realizado</h3>

                    <p className="text-sm text-muted-foreground">
                      {new Date(matchCreatedAt).toLocaleDateString("pt-BR")}
                    </p>
                  </div>
                </div>
              )}
              ...
              {firstMessageAt && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-medium">Primeira conversa</h3>

                    <p className="text-sm text-muted-foreground">
                      {new Date(firstMessageAt).toLocaleDateString("pt-BR")}
                    </p>
                  </div>
                </div>
              )}
              ...
              {firstGiftAt && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-medium">Primeiro presente enviado</h3>

                    <p className="text-sm text-muted-foreground">{new Date(firstGiftAt).toLocaleDateString("pt-BR")}</p>
                  </div>
                </div>
              )}
              ...
              {acceptedAt && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-medium">Propósito Firmado</h3>

                    <p className="text-sm text-muted-foreground">{new Date(acceptedAt).toLocaleDateString("pt-BR")}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <section className="mt-8">
          <div className="glass rounded-3xl p-6 shadow-soft">
            <div className="mb-6 flex items-center gap-2">
              ...
              <h2 className="font-semibold">Galeria do Relacionamento</h2>
            </div>
