...
GRANT SELECT ON public.name_gradients TO anon, authenticated;
GRANT INSERT, UPDATE, DELETE ON public.name_gradients TO authenticated;
GRANT ALL ON public.name_gradients TO service_role;
...
GRANT SELECT ON public.user_name_gradients TO authenticated;
GRANT ALL ON public.user_name_gradients TO service_role;
...
BEGIN
  ...
  PERFORM public.log_coin_tx(
    uid, 'name_gradient_purchase', 'out', v_price, v_new_balance,
    'Gradiente de nome: ' || COALESCE(v_name, 'Gradiente'),
    'Gradiente de Nome', _gradient_id, NULL
  );
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.name_gradients WHERE id = _gradient_id AND is_active = true
  ) THEN RAISE EXCEPTION 'name_gradient_not_found'; END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.user_name_gradients WHERE user_id = uid AND gradient_id = _gradient_id
  ) THEN RAISE EXCEPTION 'not_owned'; END IF;

  UPDATE public.profiles SET equipped_name_gradient_id = _gradient_id WHERE id = uid;
...
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'not_authenticated'; END IF;
  UPDATE public.profiles SET equipped_name_gradient_id = NULL WHERE id = uid;
  RETURN jsonb_build_object('success', true);
END;
$$;
